...
import { signInSchema, type SignInInput } from "@createconomy/utils/validation";
import { getTenantFromHostname } from "@createconomy/utils/tenant";
import { useSignIn } from "@createconomy/convex/hooks";
...
export function SignIn({
  ...
  const validateField = (name: keyof SignInInput, value: string) => {
    const result = signInSchema.safeParse({ ...formData, [name]: value });
    if (!result.success) {
  ...
  const handleInputChange = (name: keyof SignInInput, value: string) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!result.success) {
      ...
      for (const [fieldName, errorList] of Object.entries(result.error.formErrors.fieldErrors)) {
        if (fieldName && errorList) {
          errors[fieldName as keyof SignInInput] = errorList[0] || "Invalid value";
        }
      }
    ...
    try {
      ...
      const authResult: AuthResult = await signIn(tenantDomain, formData);
      ...
      if (authResult.success) {
        ...
        const errorMessage = authResult.error || "Sign in failed";
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <a
                  ...
                  className="text-sm text-[#FF8F67] hover:underline"
                >
                  Forgot password?
                </a>
              </div>
